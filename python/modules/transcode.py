...
def _ensure_start_frame_alias(options):
    """Renderer sends frame_start; FFmpeg helpers historically used start_frame."""
    if options.get('start_frame') is None and options.get('frame_start') is not None:
        options['start_frame'] = options['frame_start']
...
def generate_preview_webp(input_path, output_path, options=None, log_callback=None):
    ...
    try:
        ...
        if is_sequence:
            cmd.extend(['-start_number', str(opts.get('start_frame', 1))])
        else:
