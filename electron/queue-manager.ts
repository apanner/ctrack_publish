...
export class QueueManager {
    ...
    constructor() {
        const dbPath = path.join(app.getPath('userData'), 'ctrack_queue.db');
    ...
    addJob(job: DBJob) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(job.id, job.file_path, job.status, job.progress, job.error || null, job.project_id || null, job.shot_id || null, job.shot_code || null, job.task_id || null, job.task_name || null, job.tracking_number || null, job.meta ?? null);
    }
    ...
    addJobLog(jobId: string, message: string, extras?: Partial<Omit<DBJobEventInput, 'job_id' | 'message'>>) {
    ...
    updateJob(id: string, updates: Partial<DBJob>) {
