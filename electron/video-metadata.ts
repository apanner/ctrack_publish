/**
 * Read video metadata from a file path (Node/Electron main process).
 * Uses mp4box to parse MP4/MOV container — works for ProRes, H.264, H.265/HEVC.
 * Returns duration and derived frame count (24fps) for DB frame range.
 */
...
export async function getVideoMetadata(filePath: string): Promise<VideoMetadataResult | null> {
  ...
  // mp4box expects ArrayBuffer with fileStart (0 for whole file)
