...
export function SettingsView() {
  ...
  const handleCheckUpdates = useCallback(async () => {
    const result = await (window as any).ipcRenderer?.invoke("updater:check")
    if (result?.ok) {
      addLog("info", result.message ? `Update check: ${result.message}` : "Update check complete.")
    } else {
      addLog("warn", result?.message || "Update check failed (dev builds skip auto-update).")
    }
  }, [addLog])
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 pt-1">
              <Button
                ...
                variant="outline"
                size="sm"
                className="border-[#404040] text-gray-200 hover:bg-[#333]"
