    ...
    const startPublish = useCallback(async (jobId?: string) => {
        ...
        try {
            ...
            if (job.options?.gif !== false) {
                ...
                transcodeResult = pythonResult.transcode as { status: string; output?: string; message?: string };
                if (!transcodeResult || typeof transcodeResult !== "object") {
                    const top = pythonResult as { status?: string; message?: string };
                    throw new Error(
                        top?.message ?? (top?.status === "error" ? "Transcode failed" : "Transcode returned no result (engine response shape)")
                    );
                }
