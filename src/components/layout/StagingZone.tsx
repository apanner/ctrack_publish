...
export function StagingZone({ onDropItems, onClear, items = [] }: StagingZoneProps) {
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (paths.length === 0) {
            addLog("warn", `No valid paths found — use "Browse files" for correct paths`)
            return
        }

        const raw = await (window as any).ipcRenderer.invoke('staging:process-paths-or-folders', { paths })
        const { items: resolvedItems, unsupported } = resolveProcessResult(raw)
        ...
        if (resolvedItems.length > 1) {
            setSequencePicker({ candidates: resolvedItems, unsupported, firstPath: paths[0] })
            return
        }

        applyItems(resolvedItems, { firstPath: paths[0] })
    }, [addLog, applyItems])

    const handleBrowseFiles = useCallback(async () => {
        const raw = await (window as any).ipcRenderer?.invoke("dialog:open-files")
        const items: StagingItem[] = Array.isArray(raw) ? raw : []
        if (items.length > 0) {
            items.forEach((it) => addLog("info", `Added: ${it.fileName} (${formatBytes(it.size)})`))
            onDropItems(items, { firstPath: items[0]?.filePath })
            addLog("info", "Files staged. Click Publish to add to queue and go to Queue tab.")
        }
    }, [onDropItems, addLog])

    const handleBrowseFolder = useCallback(async () => {
        const raw = await (window as any).ipcRenderer?.invoke("dialog:open-folder-files")
        const { items: resolvedItems, unsupported } = resolveProcessResult(raw)
        ...
        if (resolvedItems.length > 1) {
            setSequencePicker({ candidates: resolvedItems, unsupported, firstPath: resolvedItems[0]?.filePath })
            return
        }

        applyItems(resolvedItems, { firstPath: resolvedItems[0]?.filePath })
    }, [addLog, applyItems])
